...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect('/dashboard/settings?error=instagram_denied');
  }
  if (!code || !state) {
    return NextResponse.redirect('/dashboard/settings?error=instagram_token_failed');
  }
  const stateData = verifyOAuthState(state);
  if (!stateData) {
    return NextResponse.redirect('/dashboard/settings?error=instagram_state_invalid');
  }
  ...
  try {
    ...
    if (page) {
      ...
      const igRes = await fetch(`https://graph.facebook.com/v18.0/${page_id}?fields=instagram_business_account&access_token=${access_token}`);
      const igData = await igRes.json();
      account_id = igData.instagram_business_account?.id || null;
    }
    // Save to Supabase
    ...
    return NextResponse.redirect(`/dashboard/settings?connected=instagram${account_id ? '' : '&warning=no_account_id'}`);
  } catch (e) {
    return NextResponse.redirect('/dashboard/settings?error=instagram_token_failed');
  }
}
